...
import ru.rknrl.castles.account.auth.Auth
...
class AuthTest extends ActorsTest {

  var authIterator = 0

  def newAuth(config: Config) = {
    authIterator += 1
    system.actorOf(Props(classOf[Auth], config), "auth-" + authIterator)
  }
