...
class ExitUnitToUnitsTest extends FlatSpec with Matchers {
  "exitUnits->units" should "work with empty list" in {
    ...
    Moving.`exitUnit→units`(List.empty, buildings, config, unitIdIterator, PlayerStateTest.playerStates, 1990).size should be(0)
  }
  ...
  "exitUnits->units" should "return units" in {
    ...
    val unitIdIterator = new UnitIdIterator
    val units = Moving.`exitUnit→units`(List(exit1, exit2), buildings, config, unitIdIterator, PlayerStateTest.playerStates, 1990).toList
